              ...
              <div className="sp-main-card" style={{ background: 'linear-gradient(135deg, #ffffff 0%, #f8fafc 100%)', padding: 'clamp(24px, 4.5vw, 36px)', boxShadow: '0 3px 14px -2px rgba(0,0,0,0.10), 0 1px 3px rgba(0,0,0,0.06)', border: '1px solid #e2e8f0', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
                ...
                <div style={{ position: 'relative', zIndex: 1 }}>
                  ...
                  <div className="sp-ref-block" style={{ padding: '0 0 clamp(16px,4vw,24px)', textAlign: 'center', margin: '0 0 clamp(8px,3vw,16px)', borderBottom: '1px solid #f1f5f9' }}>
                    ...
                    <span style={{ fontSize: 'clamp(18px,4.5vw,22px)', fontWeight: 800, color: '#1e293b', fontFamily: 'ui-monospace, SFMono-Regular, "SF Mono", Monaco, Consolas, "Liberation Mono", "Courier New", monospace', letterSpacing: '0.05em', wordBreak: 'break-all', display: 'block', padding: 'clamp(4px,1vw,6px) 0' }}>{(() => {
                      // Extract passcode from instructionRef (e.g., "HLX-27367-11415" -> "11415")
                      const ref = summary.instructionRef;
                      if (ref && typeof ref === 'string') {
                        const parts = ref.split('-');
                        if (parts.length >= 3 && ref.startsWith('HLX-')) {
                          return parts.slice(2).join('-'); // Return everything after HLX-PROSPECT-
                        }
                        return ref; // Fallback to full ref if format unexpected
                      }
                      return 'N/A';
                    })()}</span>
                  </div>
